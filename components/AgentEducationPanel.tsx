                ...
                {uploadedItems.length > 0 ? (
                  <ul className="space-y-3">
                    {uploadedItems.map((item) => (
                      <li key={item.id} className="flex items-center justify-between bg-[#21262D] p-3 rounded-md">
                        ...
                        <div className="flex items-center flex-shrink-0 ml-4">
                          ...
                          <span className={`text-xs mr-4 font-medium ${getStatusColor(item.status)}`}>{item.status}</span>
